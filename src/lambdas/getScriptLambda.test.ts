...
const makeRequest = (file: string) =>
  ({ params: { file }, locals: { appContext: { useCases: () => ({ showScript: jest.fn() }) } } } as unknown as Request);

const makeResponse = () => {
  const set = jest.fn().mockReturnThis();
  const send = jest.fn().mockReturnThis();
  const status = jest.fn().mockReturnThis();
  const res = { set, send, status, locals: { appContext: { useCases: () => ({ showScript: jest.fn() }) } } } as unknown as Response;
...
describe("getScriptLocal", () => {
  it("serves the script if found", async () => {
    ...
    expect(set).toHaveBeenCalledWith({ "Content-Type": "application/javascript" });
  ...
  it("returns 404 if script not found", async () => {
    ...
    res.locals.appContext.useCases = () => ({
      showScript: jest.fn().mockRejectedValue(new Error("File not found")),
    });

    await getScriptLocal(makeRequest("missing.js"), res);

    expect(status).toHaveBeenCalledWith(404);
    expect(set).toHaveBeenCalledWith({ "Content-Type": "text/plain; charset=UTF-8" });
  ...
  it("returns 500 for internal errors", async () => {
    ...
    expect(set).toHaveBeenCalledWith({ "Content-Type": "text/plain; charset=UTF-8" });
    expect(send).toHaveBeenCalledWith("error has occurred");
